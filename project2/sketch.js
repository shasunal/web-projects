...
//waves
// https://editor.p5js.org/pippinbarr/sketches/bgKTIXoir
let rows = 5;
...
function draw() {
  ...
  //show time
  // let now = new Date();
  // let timeStr = nf(now.getHours(),2) + ":" + nf(now.getMinutes(),2) + ":" + nf(now.getSeconds(),2);
...
function drawSunAndMoon(h) {
  ...
  if (h >= morning && h < evening) {
    ...
    let y =
      t < 0.5 ? map(t, 0, 0.5, yBottom, yTop) : map(t, 0.5, 1, yTop, yBottom);
